...
color backgroundColor  = #202127;  // dark background color
color dormantColor     = #999966;  // initial color of the map
color highlightColor   = #FFFFFF;  // color for selected points
color unhighlightColor = #DADBED;  // color for points that are not selected
color waitingColor     = #CBCBCB;  // "please type a zip code" message
color badColor         = #D30210;  // text color when nothing found
...
public void setup() {
  size(400, 520);
  //size(1275, 1650);
  //map = loadImage("maineblue.png");
  myWidth = width;
  myHeight = height;
  
    
  mapX1 = 30;
  mapX2 = myWidth - mapX1-10;
  mapY1 = 30;
  mapY2 = myHeight - mapY1;
   
  font = loadFont("ScalaSans-Regular-14.vlw");
  textFont(font);
    
  messageX = 140;
  messageY = myHeight - 20;
...
//println(pieces[NAME]);
  ...
  int numberOf = int(pieces[numSchools]);

  return new Place(zip, name, x, y,numberOf);
}
...
public void draw() {
  ...
  if (typedCount == 0) {
    fill(waitingColor);
    textAlign(LEFT);
    String message = "Maine Schools by Town!";
    // if all places are loaded
    if (placeCount == totalCount) {
      if (focused) {
	message = "Type the zip code of a town name";
      } else {
  ...
  text("zoom", myWidth - 40, myHeight - 40);
  textAlign(LEFT);
}
...
void mousePressed() {
  if ((mouseX > myWidth-100) && (mouseY > myHeight - 50)) {
...
void keyPressed() {
 /* else if ((key >= 'a' && key <='z' || key >='A' && key<='Z')) {
    if (typedCount != 15) {  // only 5 digits
...
void updateTyped() {
 ...
 // typedPartials[typedCount] = typedString.toCharArray(); //Trying to use a string
...
void calcZoom() {
  if (foundCount != 0) {
    ...
    if ((spanX != 0) && (spanY != 0)) {
      float screenAspect = myWidth / float(myHeight);
      float spanAspect = spanX / spanY;
        
      if (spanAspect > screenAspect) {
	spanY = (spanX / myWidth) * myHeight;  // wide
          
      } else {
	spanX = (spanY / myHeight) * myWidth;  // tall          
      }
